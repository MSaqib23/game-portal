...
const GenreList = ({selectedGenreId,onSelectedGenres}:Props) => {
    ...
    <List >
        ...
        <HStack marginX={-3}>
            <Image boxSize='32px' objectFit="cover" borderRadius={8} src={getCroppedImage(gen.image_background)}/>
                <Button whiteSpace='normal' textAlign='left' fontWeight={gen.id==selectedGenreId?'bold':'normal'}  variant='link' fontSize='10md' onClick={()=>onSelectedGenres(gen)}>{gen.name}</Button>
            </HStack>
        </ListItem>)}
            </List>
            </>
